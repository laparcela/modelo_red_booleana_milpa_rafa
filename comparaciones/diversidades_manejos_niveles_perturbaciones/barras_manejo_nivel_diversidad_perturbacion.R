...
for(i in 1:length(manejo)){
	for(j in c(2,5,3,4,1)){
		for(h in c(2,3,4)){
			...
			lista.perturbacion[[h-1]]<-cbind(barras_trat,barras_det)
		}
		lista.diversidad[[j]]<-do.call(cbind,lista.perturbacion)
	}
	names(lista.diversidad)<-diversidad
#Genera pdf de las proporciones...
	pdf(paste0("~/Dropbox/Chido/comparaciones/diversidades_manejos_niveles_perturbaciones/barras_manejo_nivel_diversidad_perturbacion/barras_",manejo[i],"_",nivel[k],"_vs_0.pdf"),height=10,width=18)
