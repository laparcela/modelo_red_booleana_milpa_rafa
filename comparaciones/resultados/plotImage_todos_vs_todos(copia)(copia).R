...
#axis(BELOW<-1, at=1:length(xLabels), labels=xLabels, cex.axis=0.7)
 axis(LEFT <-3, at=(c(0.5,20.5,40.5,60.5,80.5,100.5)-0.5)/4+0.5, labels=F, las= HORIZONTAL<-1,
 cex.axis=0.7,line=4.5)
 axis(LEFT <-3, at=c(10,30,50,70,90)/4, labels=c("milpa", "mzcb", "mzfre", "mz","cb" ), las= HORIZONTAL<-1, cex.axis=0.9, line=4.5,tick=F)

 axis(LEFT <-3, at=seq(0.5,25.5,1), labels=F, las= HORIZONTAL<-1,
 cex.axis=0.7)
 axis(LEFT <-3, at=seq(1, 25,1), labels=rep(c("Desyerbe", "Desyerbe \nPlaguicida", "Herbicida",  "Plaguicida \nHerbicida", "Roundup" ),5), las= 2, cex.axis=0.75,tick=F)

 axis(LEFT <-2, at=(c(0.5,20.5,40.5,60.5,80.5,100.5)-0.5)/4+0.5, labels=F, las= HORIZONTAL<-1,
 cex.axis=0.7,line=4.5)
 axis(LEFT <-2, at=c(10,30,50,70,90)/4, labels=c("cb", "mz", "mzfre", "mzcb","milpa" ), las= HORIZONTAL<-1, cex.axis=0.9, line=4.5,tick=F)

 axis(LEFT <-2, at=seq(0.5,25.5,1), labels=F, las= HORIZONTAL<-1,
 cex.axis=0.7)
 axis(LEFT <-2, at=seq(1, 25,1), labels=rep(c("Roundup", "Plaguicida \nHerbicida", "Herbicida", "Desyerbe \nPlaguicida", "Desyerbe" ),5), las= HORIZONTAL<-1, cex.axis=0.75,tick=F)
