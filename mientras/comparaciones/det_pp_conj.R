...
#								print(b)
								if(b!="NaN"){
									if(b>zeta | b<(-zeta)){
										matrizMZ[length(manejo)*(j-1)*length(perturbacion)*length(nivel)+length(perturbacion)*(i-1)*length(nivel)+length(nivel)*(h-1)+k, length(manejo)*(n-1)*length(perturbacion)*length(nivel)+length(perturbacion)*(m-1)*length(nivel)+length(nivel)*(l-1)+o]<-a[[h]][[i]][[j]][[k]]$conj[1]-a[[l]][[m]][[n]][[o]]$conj[1]
									}else{
